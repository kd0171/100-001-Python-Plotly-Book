...
# 通常の Dash アプリでは、コンポーネントはid="my-slider"のように固定文字列で識別
# 動的にグラフを追加するような場合には、トレースごとにidを振る
    # id={"type": "my_dropdown", "index": n_clicks}
# 動的生成する場合はこの index を使って追加後の挙動（MATCH / ALL など）を定義する
...
@app.callback(
    ...
    # Input({"type": "my_dropdown", "index": ALL}:ボタンで動的に増やした複数のドロップダウンの値をまとめて扱う
    prevent_initial_call=True,
)
